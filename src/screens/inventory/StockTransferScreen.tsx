...
const StockTransferScreen: React.FC = () => {
  ...
  const [transferItems, setTransferItems] = useState<TransferItem[]>([
    { item_id:null, item_name: "", available_stock: "Auto", quantity: "" },
  ]);
...
const selectItem = (index: number, item: any) => {
  ...
  updated[index] = {
    ...updated[index],
    item_id: item.id,
    item_name: item.name,        // change if your column is item_name
    available_stock: item.stock?.toString() ?? "0",
  };
  ...
  const resetForm = () => {
    setTransferDate("");
    setTransferItems([{ item_id:null, item_name: "", available_stock: "Auto", quantity: "" }]);
  };
...
const handleSave = async () => {
  ...
  if (validItems.length === 0) {
    Alert.alert("Missing Fields", "Please add at least one item");
    return;
  }
  ...
  try {
    ...
    const data=await getStockTransfers();
    setTransfers(data);
    Alert.alert("Success", "Stock Transfer saved");
